...
**Proposed Addition**: Embed hint in label string since @clack/prompts hint only shows when highlighted.

**Resolution**: Approved
**Notes**: Specified label-embedding approach for persistent hint visibility
...
**Proposed Addition**: Specify silent skip with no manifest entry, no copy, no summary line for zero-agent plugins.

**Resolution**: Approved
**Notes**: Added zero-agent edge case behavior to collection pipeline section
...
**Resolution**: Approved
**Notes**: Corrected to "matching the Claude driver pattern" with note that Codex uses two tiers
