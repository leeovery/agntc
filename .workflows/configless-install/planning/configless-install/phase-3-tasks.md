...
**Note on `detectType` config-`type` input**: Phase 2 task 2-1 wired `runAdd` to call `detectType(unitDir, { onWarn, forcePlugin })`. For the root `type: "plugin"` → error path to fire, `detectType` must receive the root config's `type` as `configType`. Phase 1 task 1-4 defined `detectType`'s `configType?` input; confirm `runAdd` forwards the root config's `type` (i.e. `configType: config?.type`) into the single root `detectType` call so a root `type:"plugin"` on a member-dirs structure reaches the `TypeConflictError` path. **If task 2-1's wiring did not already forward `configType: config?.type`, add it here** (this is the one small production change this task may need): the root `detectType` call becomes `detectType(unitDir, { onWarn, forcePlugin: options?.forcePlugin, configType: config?.type })`. This is the seam that makes a stray root `type:"plugin"` on a collection a hard error. (A root config with no `type` passes `configType: undefined` → structure stands → collection.)
...
**Do**:
- In `src/commands/add.ts` `runAdd`, confirm/ensure the single root `detectType` call forwards the root config's `type`: `detectType(unitDir, { onWarn, forcePlugin: options?.forcePlugin, configType: config?.type })`. (If Phase 2 task 2-1 already added `configType: config?.type`, this is a no-op verification; if not, add it — it is the load-bearing seam for the root `type:"plugin"` error on a collection.)
