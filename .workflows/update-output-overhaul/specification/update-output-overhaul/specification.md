...
### Grouping covers the whole manifest, before checking

Grouping runs over **every** non-local manifest entry in a single pass *before* any update check (the group-first pipeline), so a group is never fragmented by today's post-check category boundaries. Today's code checks per member and then sorts entries into separate loops (`[...updateAvailable, ...local]` and `constrainedUpdateAvailable`, `update.ts:473-504`); group-first replaces that with **group → check once → categorize members**, so entries of one shared intent that today's flow would scatter across those loops instead stay in one group and are checked together. This is a pre-resolution, *intent*-based collapse — it does **not** merge distinct intents that happen to resolve to the same commit (an exact-pin `v1.3.0` and a caret `^1.2.3` that resolves to `v1.3.0` stay separate groups; keying on the resolved commit would re-admit the race — see *Group-first pipeline*).
...
- **The clone/work step is per-group** — named once at the group level (`Updating <owner/repo> …`, the *Group label*), because the clone is genuinely one per-group action after dedup.
...
- **Header "old" ref when updating members diverge.** The *new* ref is genuinely shared (the group's resolved target); the *old* ref is per-member (each updating member's own installed `ref`/commit). When the updating members share one old ref — the common case, an atomically-added collection all at the same tag — the header shows that shared `old → new`. When their olds diverge (e.g. members manually installed at different tags, all now advancing to the target), the header shows **only the resolved target** (`◒ Updating owner/repo → v1.3.0 (N members)`) and **every** updating member carries its own `old → new` on its member line (not just outliers — the header shows no old, so each member's move must appear on its line). Up-to-date members are excluded from the count and contribute no "old".
- **Member-line version move rides a parenthetical suffix**, consistent with the dropped-agents notice: `✓ macos → claude  (v1.2.0 → v1.3.0)`. When a member also drops agents, both share the suffix: `✓ macos → claude  (v1.2.0 → v1.3.0; codex support removed by author)`. (This per-member move appears only in the divergent-old case above; in the common shared-old case the move sits on the header and member lines stay `✓ member → agents`.)
...
- **Two phases: batched check, then streamed updates.** All groups are resolved/checked up front under a single leading `Checking for updates…` spinner (per-group probes may run in parallel across distinct repos, as today's checks do via `Promise.all`); this is where each group's target and version move are resolved. Only *updatable* groups then enter the streaming phase, each with its own `Updating <repo> v… → v… (N members)` spinner in deterministic processing order. **Processing order** (used throughout this section) = manifest order: updatable groups stream in the order their first member appears in the manifest, and local group-of-one lines interleave at their own manifest positions — a stable, deterministic order the acceptance tests assert against. A group whose check finds every member non-updatable (all up-to-date / `newer-tags` / `constrained-no-match`), or whose check itself failed (`check-failed`), never clones and never emits an `Updating` spinner — it is silent in the streamed phase and appears only as its collapsed trailing line. This is why the group spinner can carry the resolved version move: the check has already resolved it before streaming begins.
...
- **Per-group collapse spans *every* trailing category.** Trailing lines collapse to **one line per group** — keyed by the grouping key `(resolvedCloneUrl, versionIntent)`, *not* the bare repo — across *all* trailing categories: `up-to-date`, out-of-constraint, `newer-tags`, `check-failed`, and `constrained-no-match` (`update.ts:533-570`). An exact-pinned 10-member collection otherwise emits 10 near-identical `newer-tags` lines — the wall, resurfacing. This falls out of group-first: the trailing categories all depend on the shared intent, so they're group-uniform (one check per group → one trailing line per group). Collapsing by *repo* instead would merge distinct-intent groups of the same repo (e.g. members pinned `@^1` and `@^2`) and silently drop one group's version info — a correctness bug, not cosmetics. Example: `owner/repo: 7 up to date` as one line, not 7.
...
- **The footer collapses per group, not per member.** Today `renderOutOfConstraintSection` emits one line per key (`summary.ts:294-306`); a major-available N-member collection (members share intent) produces N near-identical actionable lines — the "wall" Part 1 exists to kill, reappearing in the footer. **Decision:** collapse to **one line per group** (the grouping key, using the *Group label*), reusing Part 1's grouping — never per bare repo, so two distinct-intent groups of one repo keep their own current→newer pairs. The bare `npx agntc add owner/repo` re-add is *correct* at the group level — for a collection it re-adds the collection (re-selecting members at the new major), for a standalone it re-adds the plugin.
...
- **New coverage** for the grouped/dedup path: grouping by `(resolvedCloneUrl, versionIntent)` (`versionIntent = constraint ?? ref`), including a **singly-updated constrained member staying grouped** with its behind siblings (its mutated `ref` excluded from the key); one clone + one check per group; per-member categorization against the shared target; genuine-state splits; clone-failure fan-out to N `failed` outcomes with grouped rendering; per-member reinstall isolation; per-group manifest persistence; and per-group trailing collapse.

### Acceptance criteria

Observable outcomes the finished feature must satisfy:

1. A multi-member collection at one `(resolvedCloneUrl, versionIntent)` (`versionIntent = constraint ?? ref`) clones **once** and runs **one** update check for the whole group — and a constrained collection stays a single group even after one member was updated singly (its mutated `ref` is excluded from the key), that member reporting up-to-date while behind siblings update.
