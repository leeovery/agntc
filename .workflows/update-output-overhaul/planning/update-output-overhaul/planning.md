...
| Internal ID | Name | Edge Cases |
|-------------|------|------------|
| update-output-overhaul-6-1 | Single-source the sourceSubpath containment guard across the singleton and grouped clone paths | guard + resolveUpdateSourceDir (incl. PathTraversalError-vs-rethrow discrimination) lives in exactly one location; both cloneAndReinstall remote branch and reinstallMember obtain guarded sourceDir via the shared helper; escaping sourceSubpath (e.g. ../evil) rejected pre-flight on BOTH singleton and grouped paths (clone-failed, no nuke, no copy, install intact); sourceSubpath absent → guard no-ops (key-derived fallback); each call site keeps only its one-line failure mapping (raw clone-failed vs mapReinstallResultToOutcome); cloneRoot = tempDir containment boundary preserved (within-clone symlinks allowed); regression asserts both entry points reject the same subpath so one-sided divergence fails |
| update-output-overhaul-6-2 | Consolidate PluginOutcome failure/success outcome handling into co-located helpers | failed literal + "<key>: Failed — <message>" wording lives once (all six sites call failedOutcome); member.key-vs-key drift closed; updated\|refreshed success set lives once (all five sites call isSuccessOutcome, newEntry-reading sites narrow through the guard); surrounding failure-mapping (raw vs mapReinstallResultToOutcome vs clone-fatal fan-out) unchanged; identical outcome shapes + summary strings; unit test locks failedOutcome literal against drift |
| update-output-overhaul-6-3 | Carry structured fields on the updated/refreshed PluginOutcome instead of a pre-rendered summary the renderer discards | updated/refreshed outcome carries structured droppedAgents (and optionally agents); emitMemberLine composes from structured fields, oldEntry-vs-newEntry set-difference recompute eliminated (droppedAgentsFor removed if unused); summary dropped or lazy for members re-rendered by multi-member path; byte-identical output across collapsed group-of-one, multi-member, local; dropped-agents notice sourced from structured field; collapsed/local Refreshed/Updated lines unchanged |
| update-output-overhaul-6-4 | Remove dead git generality from the now-local-only update path (processUpdateForAll) | overrides param + ...overrides spread removed (sole caller streamLocalWork updated); fabricated newRef threading removed (local/refreshed direct or null to mapReinstallResultToOutcome); apologetic benign-lie comment deleted; local group-of-one produces identical refreshed outcome + summary; git-update arm of mapReinstallResultToOutcome kept reachable/unchanged via processGroupUpdate; optional rename to processLocalUpdate only if no churn; grouped git-update tests unaffected |
...
| Internal ID | Name | Edge Cases |
|-------------|------|------------|
| update-output-overhaul-7-1 | Single-source the never-downgrade constrained guard in categorizeMember | never-downgrade rule expressed once in categorizeMember (no inline re-application at any call site); constrained ref semver-above target.tag → constrained-up-to-date carrying latestOverall (not constrained-update-available), never pushed to updating on single-key or all-mode; constrained ref genuinely behind → constrained-update-available and updates; runSingleUpdate + splitMember inline isAtOrAboveVersion guards removed; single-key "already up to date" outro + all-mode upToDateOutcome(key) still fire (no user-visible change); isAtOrAboveVersion import dropped from update.ts if unused |
| update-output-overhaul-7-2 | Compute the divergent-old flag once and thread it to both header and member-line rendering | divergent-old boolean computed once in streamGroupWork (from item.updating entry.commit set); threaded to both formatGroupHeader and streamGroupMemberLines (neither derives Set-size internally); shared-old group → header move, member lines bare; divergent-old (≥2 distinct entry.commit) → target-only header + per-member old→new on every line; move renders exactly once per member across header/member pair; no other caller needs new arg; output byte-for-byte identical to today |
...
| Internal ID | Name | Edge Cases |
|-------------|------|------------|
| update-output-overhaul-8-1 | Remove group-collapse redesign's dead presentation residue | four non-actioned PluginOutcome variants (up-to-date/newer-tags/check-failed/constrained-no-match) lose the write-only summary field; updated/refreshed/failed/copy-failed/aborted/blocked/skipped-no-agents keep summary (genuinely read); splitMember + upToDateOutcome stop constructing per-member sentences (return lean {status,key}); renderOutcomeSummary trimmed to sole reachable failed case (or inlined + deleted); OutOfConstraintInfo.constraint field + its producers (groupOutOfConstraintInfo, single-key path) and test fixtures removed; CLI output byte-identical across all four categories + out-of-constraint footer; tsc clean; npm test 1729 unchanged; pure dead-code removal — no formatter/emitCollapsedGroupSummary/hasFailedOutcome/exit-accounting changes |
